...
The remaining panels are deferred on one rule: **a panel exists exactly when a producer for its metric exists.** Building them here would show flat lines meaning "no breaker" rather than "no trips", which is a worse lie than an absent panel; a test asserts each is absent until its phase arrives.

> **Updated at the Phase 3 readiness review.** The circuit state timeline and failover panels moved to **Phase 3**, not Phase 6. The roadmap's M3 exit criterion requires the timeline — *"Circuit state timeline panel shows the full shape"* — and `docs/PHASE-3-PLAN.md` had assumed Phase 2 would build it, so between the two documents nobody owned it. Phase 3 produces `keel_breaker_state` and `keel_failover_events_total`, so Phase 3 is where the rule above says those panels belong. Queue depth and cost still wait for Phase 5 and Phase 4. The absence test became `test_the_dashboard_has_a_panel_for_every_metric_something_produces`, which now asserts the rule in both directions.
